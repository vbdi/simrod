...
def get_latest_run(opt, search_dir='./runs'):
...
def compute_loss(p, targets, model):  # predictions, targets, model
    ...
    # TODO: Revert back to equal balance
    balance = [1.0, 1.0, 1.0]
...
class MyLoss(nn.Module):
    ...
    def forward(self, p, targets):  # predictions, targets, model
        ...
        tcls, tbox, indices, anchors = build_targets(p, targets, self.model)  # targets
        red = 'mean'  # Loss reduction (sum or mean)

        # Define criteria
        BCEcls = self.cls_loss(pos_weight=ft([h['cls_pw']]), reduction=red)
        BCEobj = self.obj_loss(pos_weight=ft([h['obj_pw']]), reduction=red)
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if nb:
                ...
                # Obj
                tobj[b, a, gj, gi] = (1.0 - self.gr) + self.gr * giou.detach().clamp(0).type(
...
def build_targets(p, targets, model):
    ...
    for i in range(det.nl):
        ...
        if nt:
            ...
            # print('model_anchor_t: {}'.format(model.hyp['anchor_t']))
...
def kmean_anchors(path='./data/coco128.yaml', n=9, img_size=640, thr=4.0, gen=1000, verbose=True):
    ...
    if isinstance(path, str):  # *.yaml file
        with open(path) as f:
            data_dict = yaml.safe_load(f) # , Loader=yaml.FullLoader)  # model dict
